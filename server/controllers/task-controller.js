...
const getTask = async (req, res) => {
    connection.query("SELECT * FROM tasklist WHERE taskID=?", [req.params.id], (err, task) => {
        if (err) res.status(500).json({ msg: err })
...
const deleteTask = async (req, res) => {
    connection.query("DELETE FROM tasklist WHERE taskID=?", [req.params.id], (err, task) => {
        if (err) res.status(500).json({ msg: err })
        else res.status(200).json({ status: 'deleted', deletedRows: task.affectedRows });
    })
}
const updateTask = async (req, res) => {
    console.log(req.body);
    connection.query("UPDATE  tasklist SET ?  WHERE taskID=?", [req.body, req.params.id], (err, task) => {
        if (err) res.status(500).json({ msg: err })
        else res.status(200).json({ status: 'updated', updatedRows: task.affectedRows });
    })
}
